...
main (void)
{
  ...
  if (*argv != NULL) {
    ...
    char string[7]="";   //for storing the series of characters written to the console
    char name_command[7] = "whoami";
    char exit_command[5] = "exit";
    uint8_t encripted_int;  //stores the key attained from input_getc
    char encripted_letter;  //stores the character converted from ascii(encripted_int)
    int input_length= 0;    //total length of string printed to the console
    int ENTER_CR =13;
    int NEW_LINE =10;

   while(1){
        encripted_int = input_getc();
        encripted_letter = (char) encripted_int;
        putbuf(&encripted_letter,1);

        if(encripted_int==ENTER_CR || encripted_int==NEW_LINE){

            printf("\n");
            if(input_length==6 && strcmp(name_command,string)==0){
                input_length =0;
                printf("Abdullah\n");
                printf("C69> ");

            }else if(strcmp(exit_command,string)==0 && input_length==4){
            ...
            string[input_length+1]='\0';
            string[input_length]= encripted_letter;
            input_length++;
        }
